Shader "Custom/DrawingPaper"
{
	...
	SubShader
	{
		Pass
		{
			...
			uniform float4 _PColour;
			uniform float4 _PColour2;
			...
			float4 frag(v2f i) : COLOR
			{
				...
				float2 uv = uvst; // THIS IS COMPLETELY UNNECESSARY, JUST USE UVST FOR OTHER FUNCTIONS
				...
				paper = lerp(paper.rgb, _PColour2.rgb, _Value6);
				paper = lerp(paper, _PColour.rgb, ax * _Value3);
				float pg = gg * 0.2;
				paper -= pg * 0.5;
